# MPC-JF - External media player launcher script for Jellyfin Web
### MPC-HC & MPC-BE Support, may work with other media players .exe by following step #2
## Installation Steps
...
###### To find the installed `MPC-JF.js` Browser settings → Extensions → **Violentmonkey** → Options → Go to **Installed Scripts** > ***`</>`***
...
- ``` MPC-JF.reg ``` : Do not delete. You may need to run it again if the script don't run anymore, maybe after some MPC updates.
...
- ``` MPC-JF.js ``` : backup file of the main browser script, it's also in ViolentMonkey in your browser. Use this one if an auto-update of the script have broken things for you.
